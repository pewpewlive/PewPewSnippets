...
$docs_obj = ConvertFrom-Json $raw_docs
...
function Generate-Enums {
  ...
  foreach ($enum in $enums) {
    foreach ($value in $enum.values) {
      $enums_obj[("{0}_{1}" -f $enum.name, $value)] = @{}
...
$pewpew_enums = ConvertTo-Json (Generate-Enums $pewpew.name $pewpew.enums)
$pewpew_functions = ConvertTo-Json (Generate-Functions $pewpew.name $pewpew.functions)
$fmath_functions = ConvertTo-Json (Generate-Functions $fmath.name $fmath.functions)
